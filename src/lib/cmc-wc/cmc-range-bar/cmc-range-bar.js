...
`;
    ...
    attributeChangedCallback(name, _, newValue) {
        ...
        switch (name) {
            case "min":
                ...
                this.querySelector('div[data-id="min"]').textContent = newValue;
                break;
            case "max":
                this.#max = Number(newValue);
                this.querySelector('div[data-id="max"]').textContent = newValue;
                break;
            case "value":
    ...
    set background(val) {
        this._setAttr("background", val);
    }
    set steps(val) {
        this._setAttr("steps", val);
    }
    get max() {
        return this.#max.toString(10);
    }
    set max(val) {
        this._setAttr("max", val);
    }
    get min() {
        return this.#min.toString(10);
    }
    set min(val) {
        this._setAttr("min", val);
    }
    set decimals(val) {
        this._setAttr("decimals", val);
    }
    #update() {
        if (!this.#elBar)
            return;
